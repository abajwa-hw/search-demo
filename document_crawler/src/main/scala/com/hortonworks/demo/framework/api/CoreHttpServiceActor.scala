...
import com.hortonworks.demo.framework.services.HQSearch
              ...
              pathPrefix("solr") {
                pathPrefix(coreName) {
                  ...
                  path("getUrl") {
                    ...
                    path("search") {
                      get {
                        parameters("query", "start".as[Int] ? 0, "facet.field" ? "", "highlight.field" ? "") { (query, start, facetField, highlightField) =>
                          complete {
                            def search = {
                              if (highlightField != null) {
                                val request = actorRefFactory.actorSelection("../" + name) ? HQSearch(query, start, facetField, highlightField)
                                Await.result(request, timeout.duration).asInstanceOf[String]
                              } else {
