...
class SolrServiceActor(core: String) extends Actor with ApiActor {
  ...
  implicit val solrTimeout = Timeout(7 seconds)
  ...
  protected def responseToMap(response: QueryResponse): QueryResult = {
    ...
    QueryResult(response.getResults().getNumFound(), toSeq(response.getResults()).toList, response.getResults().getStart(), facetMap.toMap)
  }

  def doSearch(query: String, filter: String, start: Integer, facetField: String): QueryResult = {
  ...
  def receive = {
    ...
    case q: QSearch => { sender() ! doSearch(q.query, null, q.start, q.facetField).toJson.toString }
    case fq: FQSearch => { sender() ! doSearch(fq.query, fq.filterQuery, fq.start, null).toJson.toString }
...
case class QueryResult(numFound: Long, results: List[JsValue], start: Long, facets: Map[String, List[Tuple2[String,Long]]])
